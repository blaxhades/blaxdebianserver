...
if [[ $tools == "y" ]]
  ...
  then
	printf "\e[1;31m[Canceled by user]\e[0m\n"
fi
sleep 1s
clear
printf "\e[1;31m                           TKJ PRODUCT\n\033[0m"
#printf "\e[1;32m[+]============================================================[+]\n\e[0m"
printf "\n"
printf "\e[1;38m[+] IP STATIC\e[0m                                             [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] DHCP SERVER\e[0m                                           [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] DNS SERVER\e[0m                                            [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] WEB SERVER\e[0m                                            [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] FTP SERVER (proftpd)\e[0m                                  [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] SSH SERVER\e[0m                                            [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] MAIL SERVER (dovecot-core & dovecot-imapd & postfix)\e[0m  [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] SQUID PROXY\e[0m                                           [\e[1;32mOK\e[0m]\n"
printf "\e[1;38m[+] SAMBA\e[0m                                                 [\e[1;32mOK\e[0m]\n\n"
#printf "\e[1;32m[+]============================================================[+]\e[0m\n"
printf "\e[1;31mDNS SERVER AND IP STATIC CONFIGURATION\e[0m\n"
#printf "\e[1;32m[+]============================================================[+]\e[0m\n\n"
...
printf "\n\n"
printf "\e[1;31mDHCP SERVER CONFIGURATION\e[0m\n"
ipcalc $ip/$prefix
read -p 'Subnet                              : ' st
read -p 'Netmask                             : ' nk
read -p 'BroadCast                           : ' bc
read -p 'Range (ex: 10.10.10.1 10.10.10.254) : ' ran
printf "\n\n"
printf "\e[1;31mVSFTPD CONFIGURATION\e[0m\n"
/etc/init.d/vsftpd restart
#printf "\e[1;32m[+]============================================================[+]\e[0m\n"
printf "\n\n"
printf "\e[1;31mWEB SERVER CONFIGURATION\n\e[0m"
...
printf "\n"
printf "\e[1;31mWEB MAIL CONFIGURATION\n\e[0m"
read -p 'Name of configuration file of mail  : ' mailconf
read -p 'Server Name                         : ' mailservername
read -p 'Document root                       : ' mailrootdir
printf "\n\n"
printf "\e[1;31mSQUID PROXY\e[0m\n"
read -p 'the name of domain you gonna block  : ' blockweb
echo "$blockweb" > /etc/squid/blacklist.txt
read -p 'the name of domain you gonna allow  : ' allowweb
echo "$allowweb" > /etc/squid/whitelist.txt
/etc/init.d/squid restart
#printf  "\e[1;32m[+]============================================================[+]\e[0m\n"
printf "\n\n"
printf "\e[1;31mSSH CONFIGURATION\e[0m\n"
...
read -p "the name of directory (security)    : " smbdirsec
mkdir $smbdirsec
echo "with security" > $smbdirsec/security.txt
read -p "the name of directory (non-security): " smbdir
mkdir $smbdir
echo "no security" > $smbdir/non-security.txt
read -p "name of user for samba : " smbuser
useradd $smbuser
smbpasswd -a $smbuser
...
acl tolak dstdomain "/etc/squid/blacklist.txt"
acl terima dstdomain "/etc/squid/whitelist.txt"

http_access deny tolak
http_access allow terima
...
cat <<EOF > /etc/apache2/sites-available/$mailconf.conf
<VirtualHost *:80>
		ServerName $mailservername
		ServerAdmin $serveradmin
		DocumentRoot /var/www/$mailrootdir
		ErrorLog ${APACHE_LOG_DIR}/error.log
		CustomLog ${APACHE_LOG_DIR}/access.log combined
</VirtualHost>
EOF
mkdir /var/www/$mailrootdir
chown -R www-data:www-data /var/www/$mailrootdir
chmod 755 /var/www/$mailrootdir

cat <<EOF > /var/www/$mailrootdir/index.html
<html>
<head>
<title> WEB MAIL CONFIGURATION WAS SUCCESS</title>
</head>
<body>
<center>
<h1> WEB MAIL CONFIGURATION WAS SUCCESSFULLY</h1>
</center>
</body>
</html>
EOF
...
EOF
cat <<EOF > /usr/share/squid-langpack/en-us/ERR_ACCESS_DENIED
<html>
    <head>
    ...
    <body>
        <center>
        <div class="box">
            UNABLE TO ACCESS <br><a href="%U" style="text-decoration: none; color: red;">%U</a>
            <br><br>
            Contact the administrator : <a style="text-decoration: none; color: red;"href="mailto:%w%W">%w</a>
        </div>
        <a class="btn" style="text-decoration: none;" href="https://www.google.com">Redirect to Google</a>
        <br><br>
        <p style="color: white">&copy; zahid [TKJ]</p>
    </center>
        <style>
            body {
...
mydestination = $myhostname, debian, $domain, localhost
relayhost =
